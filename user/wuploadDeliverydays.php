...
if($trackings != null){
	...
	if($trackingArr != null){
		foreach ($trackingArr as $trackingdata){
			...
			if($trackingInfo != null && count($trackingInfo)==3){
				$dbhelper->updateDeliveryData($trackingInfo[0], $trackingInfo[1], $trackingInfo[2]);
			}else{
				echo "<br/>数据格式不对";
			}
		}
	}
	echo "<br/><br/><br/><br/><br/><br/>更新数据记录完成";
}


$curday = date();
$pre2months = date('Y-m-d',strtotime('-2 month'));
$trackings = $wishHelper->getTrackingsFromDay($pre2months);
?>
...
<script type="text/javascript">
	function parseData(){
		...
		for(var i=0;i<trackings.length;i++){
			...
			if(curdatas.length<=5)
				break;
			var curdays = curdatas[curdatas.length-2];
			
			if($.trim(curdays).indexOf("(") ==0){
				curdays = curdays.substr(1);
				var tempdata = curdatas[0] + "|" + curdatas[3] + "|" + curdays;
				parseresult.push(tempdata);
			}
		...
		if(parseresult.length>0){
			for(var k=0;k<parseresult.length;k++){
				verifiedContent = verifiedContent + parseresult[k] + "\n"; 
				}
